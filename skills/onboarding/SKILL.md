...
Use graphify for AST-accurate structural analysis, then layer designer-specific metadata on top.

### Step 4a: Run graphify (if installed)

```bash
# Check if graphify is available
command -v graphify &>/dev/null || pip install graphifyy

# Analyze the project — generates import graph, call graph, community clusters
graphify analyze --output "$KB_DIR/graphify-cache/"

# Generate wiki articles (Wikipedia-style summaries per component)
graphify wiki --output "$KB_DIR/graphify-cache/wiki/"
```

If graphify is not available and pip fails: fall back to Step 4b manual analysis only. Note in component-graph.md: "Graph generated by manual analysis — install graphify for AST-accurate results."

### Step 4b: Translate to designer format

Graphify gives structural truth (import edges, community clusters). Layer designer meaning on top:

**From graphify output → extract:**
- All component files and their import edges (actual, not inferred)
- Community clusters → map to role groups (Layout, Navigation, Data display, Data input, Feedback, Atoms)
- Call graph → identify leaf atoms vs composite components vs pages

**Add designer metadata (graphify can't know this — read the components):**
- Which entity fields each component consumes (read props types)
- Component health: which of the 5 states are implemented (loading/error/empty/populated/partial)
- Design issues: hardcoded values, missing responsive, missing a11y
- Status: active / legacy / deprecated / duplicate

**Build the Mermaid diagram from graphify edges, not from manual inspection:**
```bash
# Read graphify's JSON graph to extract edges accurately
cat "$KB_DIR/graphify-cache/graph.json" | jq '.edges[] | "\(.source) --> \(.target)"'
```

Use the community clusters as `subgraph` groupings. Color page nodes `fill:#e0f2fe`.
...
8. Tooling gaps and recommendations — detect what is installed (see `references/tool-detection.md`); show the full tool catalog from `../../recommended-tools.md`
